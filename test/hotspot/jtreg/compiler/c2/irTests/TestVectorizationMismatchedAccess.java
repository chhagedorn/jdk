...
public class TestVectorizationMismatchedAccess {
    ...
    @IR(skip = true, counts = { IRNode.LOAD_VECTOR_L, ">=1", IRNode.STORE_VECTOR, ">=1" })
    // TODO: FAILS: adr is CastX2P(dest + 8 * (i + int_con))
    // See: JDK-8331576
    ...
    @IR(counts = { IRNode.LOAD_VECTOR_L, "=0", IRNode.STORE_VECTOR, "=0" }) // temporary
    @IR(skip = true, counts = { IRNode.LOAD_VECTOR_L, ">=1", IRNode.STORE_VECTOR, ">=1" })
    // TODO: FAILS: adr is CastX2P(dest + 8L * (i + int_con))
    // See: JDK-8331576
