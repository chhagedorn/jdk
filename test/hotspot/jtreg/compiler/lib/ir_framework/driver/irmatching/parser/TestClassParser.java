...
public class TestClassParser {
     ...
     */
    public Matchable parse(TestVmProcess testVmProcess) {
        TestMethodParser testMethodParser = new TestMethodParser(testClass);
        TestMethods testMethods = testMethodParser.parse(testVmProcess.irEncoding());
        if (testMethods.hasTestMethods()) {
            ...
            LoggedMethods loggedMethods = hotSpotPidFileParser.parse(testVmProcess.getHotspotPidFileName());
            return createTestClass(testMethods, loggedMethods, testVmProcess.testVmData());
        }
     ...
     */
    private Matchable createTestClass(TestMethods testMethods, LoggedMethods loggedMethods, TestVmData testVmData) {
        IRMethodBuilder irMethodBuilder = new IRMethodBuilder(testMethods, loggedMethods, testVmData.isNotCompilableAllowed());
        SortedSet<IRMethodMatchable> irMethods = irMethodBuilder.build(testVmData.vmInfo());
