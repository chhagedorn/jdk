 ...
 * any additional test changes (also see <i>README.md</i>):
 * <ul>
...
class Violation {
    ...
    // Even though not all tests that are callee from this @Run method are skipped, there is no way to modify the
    // code to call testSkip but avoid calling testNotSkipped(). Therefore, the entire @Run method should be excluded
    // which could be unexpected. Therefore, the IR Framework rejects this with a TestFormatException by requiring either
    // all @Test methods to be skipped or non-skipped.
