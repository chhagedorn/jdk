...
// Additionally, they must have either an UCT or a halt node on the failing path depending on where it was created.
bool TemplateAssertionPredicate::is_predicate(Node* node) {
  ...
  const bool is_template_assertion_predicate = if_node->in(1)->is_OpaqueTemplateAssertionPredicate();
  assert(!is_template_assertion_predicate || RegularPredicateWithUCT::has_valid_uncommon_trap(node) ||
         AssertionPredicateWithHalt::has_halt(node), "Template Assertion Predicate must have an UCT or Halt node");
...
// them. Additionally, they must have and halt node on the failing path.
bool InitializedAssertionPredicate::is_predicate(Node* node) {
  if (!AssertionPredicate::may_be_predicate_if(node)) {
    return false;
  }
  IfNode* if_node = node->in(0)->as_If();
  const bool is_initialized_assertion_predicate = if_node->in(1)->is_OpaqueInitializedAssertionPredicate();
  assert(!is_initialized_assertion_predicate || AssertionPredicateWithHalt::has_halt(node),
         "Initialized Assertion Predicate must have a Halt node");
