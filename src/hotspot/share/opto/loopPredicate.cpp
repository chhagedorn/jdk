...
void PhaseIdealLoop::get_assertion_predicates(Node* predicate, Unique_Node_List& list, bool get_opaque) {
  ...
  assert(iff->is_ParsePredicate(), "must be parse predicate");
...
void PhaseIdealLoop::clone_parse_and_assertion_predicates_to_unswitched_loop(IdealLoopTree* loop, Node_List& old_new,
  ...
  if (loop_limit_check_predicate_block != nullptr && !head->is_CountedLoop()) {
...
// Clone Parse Predicate and Template Assertion Predicates of a Loop Predication related Predicates Block.
void PhaseIdealLoop::clone_loop_predication_predicates_to_unswitched_loop(IdealLoopTree* loop, const Node_List& old_new,
                                                                          const RegularPredicateBlock* regular_predicate_block,
                                                                          Deoptimization::DeoptReason reason,
                                                                          IfProjNode*& iffast_pred,
                                                                          IfProjNode*& ifslow_pred) {
  if (regular_predicate_block != nullptr) {
    clone_parse_predicate_to_unswitched_loops(regular_predicate_block, reason, iffast_pred, ifslow_pred);

    clone_assertion_predicates_to_unswitched_loop(loop, old_new, reason,
                                                  regular_predicate_block->parse_predicate_success_proj(), iffast_pred,
                                                  ifslow_pred);
  }
}

void PhaseIdealLoop::clone_parse_predicate_to_unswitched_loops(const RegularPredicateBlock* regular_predicate_block,
                                                               Deoptimization::DeoptReason reason,
                                                               IfProjNode*& iffast_pred, IfProjNode*& ifslow_pred) {
  ParsePredicateSuccessProj* parse_predicate_proj = regular_predicate_block->parse_predicate_success_proj();
  iffast_pred = clone_parse_predicate_to_unswitched_loop(parse_predicate_proj, iffast_pred,
                                                         reason, false);
  check_cloned_parse_predicate_for_unswitching(iffast_pred, true);

  ifslow_pred = clone_parse_predicate_to_unswitched_loop(parse_predicate_proj, ifslow_pred,
                                                         reason, true);
...
bool PhaseIdealLoop::can_create_loop_predicates(const RegularPredicateBlock* profiled_loop_predicate_block) const {
