...
void PhaseIdealLoop::clone_parse_and_assertion_predicates_to_unswitched_loop(IdealLoopTree* loop, Node_List& old_new,
  ...
  if (loop_predicate_proj != nullptr) {
    // Clone Parse Predicate and Template Assertion Predicates of the Loop Predicates Block.
  ...
  if (profiled_loop_predicate_proj != nullptr) {
    // Clone Parse Predicate and Template Assertion Predicates of the Profiled Loop Predicates Block.
...
// Skip over all predicates in a predicate block starting at the predicate projection 'node'. Return the first node not
// belonging to a predicate block anymore (i.e. entry into the first predicate in the predicate block).
...
// Find next predicate in a Regular Predicate Block.
