...
Node* OpaqueTemplateAssertionPredicateNode::Identity(PhaseGVN* phase) {
  if (phase->C->post_loop_opts_phase()) {
    ...
    // a loop during loop opts. They are not used anymore once loop opts are over and thus can be removed. They feed
    // into a BoolNode and can thus be replaced by true to let the Template Assertion Predicate fold away (the success
    // path is always the true path).
